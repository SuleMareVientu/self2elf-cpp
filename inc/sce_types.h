...
public:
    ...
    KeyEntry get(KeyType keytype, SceType scetype, uint64_t sysver = -1, int keyrev = -1, SelfType selftype = SelfType::NONE) {
        ...
        if (key_type == store.end()) {
            fprintf(stderr, "Error: Cannot find any keys for this key type\n");
            return empty_keys;
        }
        auto sce_type = key_type->second.find(scetype);
        if (sce_type == key_type->second.end()) {
            fprintf(stderr, "Error: Cannot find any keys for this SCE type\n");
            return empty_keys;
        }
        auto self_type = sce_type->second.find(selftype);
        if (self_type == sce_type->second.end()) {
            fprintf(stderr, "Error: Cannot find any keys for this SELF type\n");
            return empty_keys;
        }
        ...
        fprintf(stderr, "Error: Cannot find key/iv for this SCE file\n");
        return empty_keys;
    }
};
...
public:
    ...
    explicit SceHeader(const char *data) {
        ...
        if (magic != SCE_MAGIC) {
            fprintf(stderr, "Error: Invalid SCE magic\n");
        }
        if (version != 3) {
            fprintf(stderr, "Error: Invalid SCE version\n");
        }
...
public:
    ...
    explicit MetadataInfo(const char *data) {
        ...
        if (pad0 != 0 || pad1 != 0 || pad2 != 0 || pad3 != 0)
            fprintf(stderr, "Error: Invalid metadata info padding (decryption likely failed)\n");
    }
};
